...
<body class="bg-[#f8fafc] font-sans text-slate-900 overflow-x-hidden">
    ...
    <div class="min-h-screen flex" x-data="{ sidebarOpen: false }">
        ...
        <div x-show="sidebarOpen" x-transition.opacity x-cloak
            class="fixed inset-0 z-40 bg-slate-950/60 lg:hidden backdrop-blur-sm" @click="sidebarOpen = false">
        </div>
        ...
        <aside
            ...
            <div class="flex items-center justify-between border-b border-slate-800/60 px-6 py-6 min-h-[90px]">
                <div class="flex items-center gap-3">
                    <div
                        class="flex h-10 w-10 items-center justify-center rounded-xl bg-white p-1.5 shadow-lg shadow-blue-500/10">
                        <img src="{{ asset('images/ephac-logo.png') }}" alt="Logo" class="h-full w-auto">
                    </div>
                    <span class="text-xl font-bold tracking-tight text-white uppercase">EPHAC<span
                            class="text-blue-500">.</span></span>
                </div>
                <button type="button" class="rounded-xl p-2 text-slate-400 hover:bg-slate-800 lg:hidden"
            ...
            <div class="flex-1 overflow-y-auto px-4 py-8 custom-scrollbar">
                ...
                <!-- Group: Main Content -->
                <p class="mb-4 px-4 text-[10px] font-bold uppercase tracking-widest text-slate-500">Content Management
                </p>
                ...
                <nav class="space-y-1.5">
                    ...
                    <a href="{{ route('admin.about.cms') }}"
                        ...
                        <i class="fa-solid fa-circle-info w-6 text-center text-lg"></i>
                    ...
                    <a href="{{ route('admin.products.cms') }}"
                        class="group flex items-center gap-3 rounded-xl px-4 py-3 text-slate-400 transition-all duration-200 hover:bg-blue-600/10 hover:text-white
                        {{ request()->routeIs('admin.products.cms') || str_starts_with(request()->route()->getName() ?? '', 'admin.product') ? 'bg-blue-600/15 text-blue-400 ring-1 ring-blue-500/30' : '' }}">
                        <i class="fa-solid fa-boxes-stacked w-6 text-center text-lg"></i>
                        <span class="font-semibold text-sm">Product Page</span>
                        @if (request()->routeIs('admin.products.cms') || str_starts_with(request()->route()->getName() ?? '', 'admin.product'))
                            <div class="ml-auto h-1.5 w-1.5 rounded-full bg-blue-500"></div>
                        @endif
                    </a>
                    ...
                    <a href="{{ route('admin.activities') }}"
                        class="group flex items-center gap-3 rounded-xl px-4 py-3 text-slate-400 transition-all duration-200 hover:bg-blue-600/10 hover:text-white
                        {{ request()->routeIs('admin.activities') ? 'bg-blue-600/15 text-blue-400 ring-1 ring-blue-500/30' : '' }}">
                        <i class="fa-solid fa-calendar-days w-6 text-center text-lg"></i>
                        <span class="font-semibold text-sm">Activities Page</span>
                    ...
                    <a href="#"
                        class="group flex items-center gap-3 rounded-xl px-4 py-3 text-slate-400 transition-all duration-200 hover:bg-blue-600/10 hover:text-white">
                        <i class="fa-solid fa-hand-holding-medical w-6 text-center text-lg"></i>
                        <span class="font-semibold text-sm">Services Page</span>
                    </a>
                    ...
                    <a href="#"
                        ...
                        <span class="font-semibold text-sm">Insight Page</span>
                    </a>

                </nav>

                <!-- System Control -->
                <p class="mb-4 mt-10 px-4 text-[10px] font-bold uppercase tracking-widest text-slate-500">System Control
                </p>
                <nav class="space-y-1.5">
                    <a href="{{ route('admin.setting.cms') }}"
                        class="group flex items-center gap-3 rounded-xl px-4 py-3 text-slate-400 transition-all duration-200 hover:bg-blue-600/10 hover:text-white">
                        <i class="fa-solid fa-cog w-6 text-center text-lg"></i>
            ...
            <div class="p-4">
                <div class="rounded-2xl bg-slate-800/40 border border-slate-700/50 p-4">
                    <div class="flex items-center gap-3">
                        <div class="relative">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-sm font-bold text-white shadow-lg">
                                E
                            </div>
                            <div
                                class="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-[#0f172a] bg-green-500">
                            </div>
                        </div>
                        ...
                        <div>
                            @auth
                                <form id="logout-form" action="{{ route('logout') }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit"
                                        class="flex items-center gap-2 hover:text-gray-300 rounded-lg px-4 py-2 text-sm font-semibold text-slate-600  transition">
            ...
            <header class="sticky top-0 z-30 border-b border-slate-200 glass-header">
                <div class="flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8 h-[90px]">
                    <div class="flex items-center gap-4">
                        <button type="button"
                            class="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 transition lg:hidden"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @auth
                            <form id="logout-form" action="{{ route('logout') }}" method="POST" class="inline">
                                @csrf
                                <button type="submit"
                                    class="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 transition">
                                    <i class="fa-solid fa-arrow-right-from-bracket text-xs"></i>
                                    <span>Leave</span>
                                </button>
                            </form>
